...
public class HashMapBefore<K,V> implements Map<K,V>{
    private int size;
    private static final boolean RED = false;
    private static final boolean BLACK = true;
    private Node<K,V>[] table;
    private static final int DEFAULT_CAPACITY = 1 << 4;
    public HashMapBefore() {
...
////            afterAdd(grand);
    ...
    private int compare(K k1,K k2,int h1,int h2){
        ...
        // 说明相等了,
        // 然后，其实这种方法也是不行的，因为，这内存地址比的是int值吧，如果你规定，两个对象的
        // value相等，两个对象就相等，然后你传参数的时候就用new ,虽然你知道，比如1在2的右边
        // 但是你知道内存是怎么分配的吗？如果一直往左边走，那就永远也找不到啊！那你说，往右边走行不行呢？
        // 也不行的，你往右边走的时候，万一你的key是在右子树的左子树呢？所以，这个时候，万一又大了，
        //又是永远也找不到了。
